...
export default function QuestionDetailPage() {
  ...
  console.log('QuestionDetailPage - location:', location);
  console.log('QuestionDetailPage - location.state:', location.state);
  
  const state = location.state as { question: Question; userAnswer: number; isReviewMode?: boolean } | null;
  
  console.log('QuestionDetailPage - state:', state);
  
  const question = state?.question;
  const previousUserAnswer = state?.userAnswer ?? -1;
  const isReviewMode = state?.isReviewMode || false;

  console.log('QuestionDetailPage - question:', question);
  console.log('QuestionDetailPage - isReviewMode:', isReviewMode);

  const [selectedAnswer, setSelectedAnswer] = useState<number | null>(null);
  const [showExplanation, setShowExplanation] = useState(!isReviewMode);

  useEffect(() => {
    console.log('QuestionDetailPage mounted');
    console.log('Data received:', { question, previousUserAnswer, isReviewMode });
  }, []);

  // データがない場合
  if (!question) {
    console.error('QuestionDetailPage - No question data!');
    return (
      <div className="min-h-screen bg-gray-50 py-8">
        <div className="max-w-4xl mx-auto px-4">
          <div className="bg-white rounded-xl shadow-lg p-8">
            <h2 className="text-2xl font-bold text-red-600 mb-4">⚠️ データエラー</h2>
            <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-4">
              <p className="text-gray-800 mb-2">問題データが見つかりませんでした。</p>
              <p className="text-sm text-gray-600 mb-4">以下のデバッグ情報を確認してください：</p>
              <div className="bg-white p-4 rounded border">
                <p className="text-xs font-mono mb-2"><strong>location.pathname:</strong> {location.pathname}</p>
                <p className="text-xs font-mono mb-2"><strong>location.state:</strong> {JSON.stringify(location.state)}</p>
                <p className="text-xs font-mono"><strong>state:</strong> {JSON.stringify(state)}</p>
              </div>
            </div>
            <button
              onClick={() => navigate(-1)}
  ...
  const handleSubmitAnswer = () => {
    console.log('Submit answer:', selectedAnswer);
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-4xl mx-auto px-4">
        ...
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-4">
          <p className="text-xs font-mono">✅ QuestionDetailPage loaded successfully</p>
          <p className="text-xs font-mono">Question ID: {question.id}</p>
          <p className="text-xs font-mono">Review Mode: {isReviewMode ? 'Yes' : 'No'}</p>
          <p className="text-xs font-mono">Show Explanation: {showExplanation ? 'Yes' : 'No'}</p>
        </div>
