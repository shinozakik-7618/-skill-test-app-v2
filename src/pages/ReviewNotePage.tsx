...
export default function ReviewNotePage() {
  ...
  useEffect(() => {
    console.log('🔄 ReviewNotePage: マウントまたは更新');
    loadReviewNotes();
    
    // ページが表示されるたびにデータを再読み込み
    const handleFocus = () => {
      console.log('🔄 ページフォーカス: データ再読み込み');
      loadReviewNotes();
    };
  ...
  const loadReviewNotes = () => {
    ...
    console.log('📚 全ノート数:', notes.length);
    ...
    console.log('📚 カテゴリ別:', Object.entries(grouped).map(([cat, items]) => `${cat}: ${items.length}`).join(', '));
    console.log('📚 表示中のノート:', notes.length);
  ...
  const getQuestionText = (questionId: string): string => {
    console.log('🔍 getQuestionText: allQuestionsの型:', typeof allQuestions, Array.isArray(allQuestions));
    console.log('🔍 getQuestionText: allQuestions.length:', allQuestions.length);
