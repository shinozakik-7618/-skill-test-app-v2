      ...
      try {
        ...
        setTestResults(currentResults => {
          ...
          if (finalResults.length > 0) {
            ...
            // 🔧 バグ修正: テスト終了時に復習ノートを一括更新
            // 間違えた問題だけを復習ノートに追加し、正解した問題は削除
            finalResults.forEach(result => {
              const question = questions.find(q => q.id === result.questionId);
              if (question) {
                if (!result.isCorrect) {
                  addToReviewNote(question.id, question.category, question.question);
                } else {
                  removeFromReviewNote(question.id);
                }
              }
            });
          }
