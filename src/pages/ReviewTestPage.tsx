...
export default function ReviewTestPage() {
  ...
  const handleSubmitAnswer = () => {
    ...
    console.log('📝 Step 1: saveTestResult を呼び出します...');
    try {
      // saveTestResultは配列を期待しているので、[result]として渡す
      saveTestResult([result]);
      console.log('✅ Step 1: saveTestResult 成功');
    } catch (error) {
      console.error('❌ Step 1: saveTestResult でエラー:', error);
    }
    
    console.log('📝 Step 2: 復習ノートの更新...');
    // 不正解の場合は復習ノートに追加（回数を増やす）
    if (!isCorrect) {
      try {
        console.log('📝 Step 2a: addToReviewNote を呼び出します...');
        addToReviewNote(currentQuestion.id, currentQuestion.category, currentQuestion.question);
        console.log('✅ Step 2a: addToReviewNote 成功');
      } catch (error) {
        console.error('❌ Step 2a: addToReviewNote でエラー:', error);
      }
    } else {
      try {
        console.log('📝 Step 2b: removeFromReviewNote を呼び出します...');
        removeFromReviewNote(currentQuestion.id);
        console.log('✅ Step 2b: removeFromReviewNote 成功');
      } catch (error) {
        console.error('❌ Step 2b: removeFromReviewNote でエラー:', error);
      }
    }
    
      console.log('📝 Step 3: 状態を更新します...');
      setIsAnswerSubmitted(true);
      setShowExplanation(true);
      console.log('✅ Step 3: 状態更新成功');
    } catch (error) {
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <h2 className="text-2xl font-bold text-gray-900 mb-2">データエラー</h2>
          <p className="text-gray-600 mb-4">問題の選択肢が正しく読み込めませんでした。</p>
          <button
