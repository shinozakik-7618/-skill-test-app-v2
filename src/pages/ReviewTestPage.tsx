...
export default function ReviewTestPage() {
  ...
  const handleSubmitAnswer = () => {
    ...
    try {
      ...
      console.log('✅ 結果を一時保存 (' + (testResults.length + 1) + '/' + questions.length + '):', result.questionId);
  ...
  const moveToNextQuestion = () => {
    if (currentQuestionIndex < questions.length - 1) {
      ...
      try {
        ...
        console.log('📊 テスト終了 - 全結果を保存:', {
          問題数: finalResults.length,
          正解数: finalResults.filter(r => r.isCorrect).length
        });
        
        // saveTestResult は配列を受け取り、自動的に recordLearningHistory も呼ばれる
        if (finalResults.length > 0) {
          saveTestResult(finalResults);
          console.log('✅ saveTestResult 完了');
        }
      } catch (error) {
