...
export default function LearningCalendarPage() {
  ...
  const handleDateClick = (dateStr: string) => {
    console.log('🔍 [DEBUG] 日付クリック:', dateStr);
    console.log('🔍 [DEBUG] 現在のselectedDate:', selectedDate);
    
    if (selectedDate === dateStr) {
      console.log('🔍 [DEBUG] 同じ日付なので選択解除');
      setSelectedDate(null);
    } else {
      console.log('🔍 [DEBUG] 新しい日付を選択:', dateStr);
  ...
  useEffect(() => {
    console.log('🔍 [DEBUG] selectedDate変更:', selectedDate);
    console.log('🔍 [DEBUG] selectedHistory:', selectedHistory);
            ...
            {calendar.map((date, index) => {
              ...
              return (
                <button
                  key={dateStr}
                  onClick={() => {
                    console.log('🔍 [DEBUG] ボタンクリック検出:', dateStr);
