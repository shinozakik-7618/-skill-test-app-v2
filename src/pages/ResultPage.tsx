...
import { getTestResultById, TestResult, Question, addToReviewNote } from '../utils/storage';
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8 px-4">
      <div className="max-w-4xl mx-auto">
            ...
            {incorrectCount > 0 && (
              <button
                onClick={() => {
                  ...
                  if (incorrectSection) {
                    incorrectSection.scrollIntoView({ behavior: 'smooth', block: 'start' });
                  }
                }}
        ...
        {/* 間違えた問題セクション */}
        {incorrectCount > 0 && (
          <div id="incorrect-questions" className="mb-8">
            <h2 className="text-2xl font-bold text-red-600 mb-4">❌ 間違えた問題 ({incorrectCount}問)</h2>
            <div className="bg-white rounded-xl shadow-lg p-6">
              <p className="text-gray-600 mb-4">クリックして復習しましょう！もう一度考えて回答できます。</p>
              <div className="space-y-3">
                {questions.map((question, index) => {
                  ...
                  return (
                    <div 
                      key={question.id} 
                      className="flex items-center p-4 border-2 border-red-200 rounded-lg cursor-pointer hover:border-red-400 hover:bg-red-50 transition-all"
                    ...
                    >
                      <div className="flex-shrink-0 w-10 h-10 rounded-full bg-red-100 text-red-600 flex items-center justify-center font-bold mr-4">
                        ✗
                      </div>
                      <div className="flex-grow">
                        <div className="text-sm text-gray-500">問題 {index + 1}</div>
                        <div className="text-gray-900 font-medium line-clamp-2">{question.question}</div>
                      </div>
                      <div className="flex-shrink-0 text-red-600 ml-4">
                        →
                      </div>
                    </div>
                  );
