...
export default function TestPage() {
  ...
  const startTest = (mode: FilterMode) => {
    ...
    // 過去の回答履歴を取得
    const testHistory = getTestResultsByCategory(decodedCategory);
    const answeredQuestionIds = new Set(testHistory.map(r => r.questionId));
    const incorrectQuestionIds = new Set(
      testHistory.filter(r => !r.isCorrect).map(r => r.questionId)
    );
