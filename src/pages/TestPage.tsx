...
export default function TestPage() {
  ...
  const startTest = (mode: FilterMode) => {
    ...
    // カテゴリーの全問題を取得
    console.log('✅ [DEBUG] カテゴリー:', decodedCategory);
    const categoryQuestions = getQuestionsByCategory(decodedCategory);
    console.log('✅ [DEBUG] 取得した問題数:', categoryQuestions.length);
    console.log('✅ [DEBUG] 最初の問題:', categoryQuestions[0]);
  ...
  const handleAnswerSelect = (answerIndex: number) => {
    console.log('✅ [DEBUG] 選択肢がクリックされました:', answerIndex);
    setSelectedAnswer(answerIndex);
    console.log('✅ [DEBUG] selectedAnswerが更新されました:', answerIndex);
  };
