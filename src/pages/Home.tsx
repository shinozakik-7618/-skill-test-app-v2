...
const categories: { name: Category; icon: string; questionsCount: number }[] = [
  ...
  { name: 'AI・DX基礎知識', icon: '🤖', questionsCount: 10 }, // 🆕 10問に更新
];
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100">
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8">
          ...
          >
            <div className="text-4xl mb-2">📖</div>
            <div className="text-sm font-semibold text-gray-700">復習ノート</div>
          </button>
          <button
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {categories.map((category) => (
              <button
                key={category.name}
                onClick={() => navigate(`/mode-select/${encodeURIComponent(category.name)}`)}
