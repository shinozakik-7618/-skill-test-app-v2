...
export default function AdminPage() {
  ...
  useEffect(() => {
    if (activeTab === 'stats') {
      ...
      try {
        ...
        console.log('📊 localStorage.getItem("testResults"):', rawData ? '存在する' : '存在しない');
        
        if (!rawData) {
          console.log('📊 testResults が見つかりません。test_results を試します...');
          const altData = localStorage.getItem('test_results');
          console.log('📊 localStorage.getItem("test_results"):', altData ? '存在する' : '存在しない');
  ...
  const processTestResults = (results: any[]) => {
    ...
    console.log('📊 配列の長さ:', Array.isArray(results) ? results.length : 'not an array');
    ...
    results.forEach((r: any) => {
      ...
      questions += total;
      correct += score;
      console.log(`📊 テスト ${r.id}: ${score}/${total}`);
