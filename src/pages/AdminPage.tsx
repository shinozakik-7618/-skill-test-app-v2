...
export default function AdminPage() {
  ...
  const handleDataCleanup = () => {
    if (!confirm('データクリーンアップを実行しますか?\n\n重複したテスト結果を検出して削除します。この操作は取り消せません。')) {
      return;
    }
    ...
    try {
      ...
      alert(`データクリーンアップ完了!\n\n重複検出: ${duplicates.length}件\n削除: ${duplicates.length}件\n残り: ${cleanedResults.length}件\n\n統計を再読み込みしています...`);
  ...
  const handleDeleteByDate = () => {
    ...
    if (confirm(`${selectedDate}のデータを削除しますか？`)) {
  ...
  const handleClearAllData = () => {
    if (confirm('本当に全データを削除しますか？この操作は取り消せません。')) {
      if (confirm('再確認: 全データを削除します。よろしいですか？')) {
        clearAllData();
        alert('全データを削除しました');
        loadStatsData();
      }
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="flex space-x-1 mb-6 border-b">
          <button
            ...
            className={`px-6 py-3 font-semibold transition-colors ${activeTab === 'stats' ? 'border-b-2 border-blue-500 text-blue-600' : 'text-gray-600 hover:text-gray-900'}`}
          >
            📈 成績管理
          </button>
          <button
            onClick={() => setActiveTab('data')}
            className={`px-6 py-3 font-semibold transition-colors ${activeTab === 'data' ? 'border-b-2 border-blue-500 text-blue-600' : 'text-gray-600 hover:text-gray-900'}`}
          >
            📊 データ管理
          </button>
          <button
            onClick={() => setActiveTab('creator')}
            className={`px-6 py-3 font-semibold transition-colors ${activeTab === 'creator' ? 'border-b-2 border-blue-500 text-blue-600' : 'text-gray-600 hover:text-gray-900'}`}
          >
            ✏️ 問題作成
          </button>
        </div>
