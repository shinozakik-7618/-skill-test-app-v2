...
export default function CalendarPage() {
  ...
  const getAccuracyColor = (accuracy: number) => {
    if (accuracy >= 80) return { bg: 'bg-green-500', text: 'text-green-500', border: 'border-green-300' };
    if (accuracy >= 50) return { bg: 'bg-yellow-500', text: 'text-yellow-500', border: 'border-yellow-300' };
    return { bg: 'bg-red-500', text: 'text-red-500', border: 'border-red-300' };
  };
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 mb-8">
          ...
          <div className="grid grid-cols-7 gap-2">
            {calendarDays.map((day) => {
              ...
              const colorScheme = hasResults ? getAccuracyColor(accuracy) : null;
                ...
                <button
                  ...
                  className={`aspect-square p-2 rounded-lg border-2 transition-all relative ${
                    !isCurrentMonth
                      ...
                      : hasResults
                      ? `${colorScheme?.border} hover:bg-gray-50`
                ...
                >
                  ...
                  {hasResults && isCurrentMonth && (
                    <div className="text-xs mt-1">
                      <div className={`${colorScheme?.text} font-semibold`}>
                        {accuracy.toFixed(0)}%
                      </div>
                    </div>
                  )}
        ...
        {selectedDateResults && (
          <div className="bg-white rounded-xl shadow-lg p-6">
            ...
            <div className="grid grid-cols-3 gap-4 mb-6">
              ...
              <div className="text-center p-4 bg-purple-50 rounded-lg">
                <div className="text-2xl font-bold text-purple-600">
                  {((selectedDateResults.correct / selectedDateResults.total) * 100).toFixed(0)}%
                </div>
            ...
            <div className="space-y-2">
              <h4 className="font-semibold text-gray-900 mb-2">詳細結果:</h4>
              {selectedDateResults.results.map((result, index) => (
                ...
                >
                  ...
                  <div className="text-xs text-gray-600 mt-1">
                    カテゴリ: {result.category} | 所要時間: {result.timeSpent}秒
                  </div>
                </div>
              ))}
            </div>
